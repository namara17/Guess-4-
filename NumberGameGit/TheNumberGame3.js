 $(document).ready(function() {   // THE NUMBER GAME 
   $('#commBox').hide();		 // initialize screen
   $('#feedback').hide();
   $('.guessesLeft').hide();
 ...
 function newGame() {			 // begin game setup
...
// $('#rules').show();
 ...
 var y=0;		 
 remaining = 12; 
 myNum =[ Math.floor(Math.random()*10),0,0,0];	 // generate number
 myNum2 = "";
   for (var i=1; i<4; i++)
     {
     y = Math.floor(Math.random()*10); 	
     for (var k=0; k<i; k++)	   // make sure digits are unique
	    {
         if (y === myNum[k])
	       {
	       i--;
     ...
     // EVALUATE USER GUESS
      ...
      for (var m=0; m<4 ; m++)
        ...
        for (var loop=0; loop<4; loop++)
           {   
	       if (myNum[m] === userGuessArray[loop])	  // test each digit userGuess with myNum
              {
